...
const NETWORK_NODES: Record<string, AgentNodePos> = {
  ...
  agent_coordinator: { x: 340, y: 175, id: 'agent_coordinator', name: 'COORDINATOR', shortName: 'Coordinator', icon: '🧠', color: '#A99BC9' }, // VISUALLY CENTRAL
  agent_occupancy: { x: 140, y: 280, id: 'agent_occupancy', name: 'OCCUPANCY', shortName: 'Occupancy', icon: '👥', color: '#E6B85C' },
  agent_security: { x: 340, y: 310, id: 'agent_security', name: 'SECURITY', shortName: 'Security', icon: '🛡️', color: '#7AE04C' },
  agent_ethical_priority: { x: 540, y: 280, id: 'agent_ethical_priority', name: 'ETHICAL PRIORITY', shortName: 'Ethical', icon: '❤️', color: '#E0B7C9' },
...
export const AgentNetworkView: React.FC<AgentNetworkProps> = ({
  ...
  return (
    <div className="bg-[#292733] text-[#F3F3F3] p-5 rounded-[8px] border border-[#423F4F] shadow-md space-y-3 font-sans h-full flex flex-col">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#565E75]/40 pb-3 font-mono-tech">
        ...
        {activeSenderId && activeReceiverId ? (
          <div className="flex items-center gap-1.5 text-xs bg-[#A99BC9]/20 px-2.5 py-1 rounded border border-[#A99BC9] text-[#A99BC9] font-bold animate-pulse">
            ...
            <span>PACKET IN TRANSIT: {activeMessageType || 'MESSAGE_DELIVERY'}</span>
          </div>
        ) : (
      ...
      <div className="relative w-full min-h-[360px] flex-1 bg-[#1F2028] rounded-[6px] border border-[#423F4F]/60 overflow-hidden flex items-center justify-center">
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 680 370">
          <defs>
          ...
          {CONNECTIONS.map(([idA, idB], idx) => {
            ...
            // Determine flow direction from sender to receiver
            ...
            return (
              <g key={idx}>
                ...
                <line
                  ...
                  x2={nodeB.x}
                  y2={nodeB.y}
                  stroke={isCurrentActivePath ? '#E6B85C' : '#423F4F'}
                ...
                {/* ANIMATED PACKET BEAM (Moving from sender to receiver) */}
                {isCurrentActivePath && (
                  <g>
                    {/* Glowing outer beam */}
                    <circle r="8" fill="#E6B85C" opacity="0.6" filter="url(#glow)">
                      <animateMotion
                    ...
                    {/* Inner high-intensity packet core */}
          ...
          {Object.values(NETWORK_NODES).map((node) => {
            ...
            const isDimmed = hasActiveComm && !isSender && !isReceiver;
            ...
            if (isSender) {
              statusLabel = currentStage === 'THINKING' ? 'THINKING...' : 'SENDING...';
              statusColor = '#E6B85C';
            } else if (isReceiver) {
            ...
            const outerRadius = isCoordinator ? 32 : isSender || isReceiver ? 28 : 22;
            ...
            return (
              ...
              >
                ...
                <circle
                  r={outerRadius}
                  fill={node.color}
                  fillOpacity={isSender || isReceiver ? 0.4 : isCoordinator ? 0.25 : 0.15}
                  stroke={isSender ? '#E6B85C' : isReceiver ? '#7AE04C' : isCoordinator ? '#A99BC9' : node.color}
                  strokeWidth={isSender || isReceiver ? 3.5 : isCoordinator ? 2.5 : 1.5}
                  className={isSender || isReceiver ? 'animate-pulse' : ''}
                  filter={isSender || isReceiver ? 'url(#strongGlow)' : isCoordinator ? 'url(#glow)' : 'none'}
                />
                ...
                <circle
                  r={innerRadius}
                  fill="#292733"
                  stroke={isSender ? '#E6B85C' : isReceiver ? '#7AE04C' : node.color}
                  strokeWidth={2}
                />
                ...
                {/* Active Status Badge Below Name */}
                <text
                  y={outerRadius + 26}
                  textAnchor="middle"
                  fill={statusColor}
